...
class TimeSeriesDataset:
    ...
    def load_data(config):
        ...
        df_sird = df_sird[['suscettibili', 'totale_positivi', 'dimessi_guariti', 'deceduti']]
    ...
    def split_data(sequences, labels, sird_initial, sird_final):
        x_train, x_test, y_train, y_test, sird_initial_train, sird_initial_test, sird_final_train, sird_final_test = train_test_split(
            sequences, labels, sird_initial, sird_final, test_size=0.3, random_state=42, shuffle=False
        )

        TimeSeriesDataset.train_dataset = TensorDataset(
            torch.tensor(x_train), torch.tensor(y_train), torch.tensor(sird_initial_train), torch.tensor(sird_final_train)
        )
        TimeSeriesDataset.test_dataset = TensorDataset(
            torch.tensor(x_test), torch.tensor(y_test), torch.tensor(sird_initial_test), torch.tensor(sird_final_test)
        )